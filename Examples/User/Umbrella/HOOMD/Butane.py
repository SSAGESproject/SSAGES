...
logger = hoomd.analyze.log(
    filename="dump.log",
    quantities=['temperature', 'pressure', 'volume', 'kinetic_energy',
                'potential_energy', 'bond_harmonic_energy',
                'angle_harmonic_energy', 'dihedral_opls_energy',
                'pair_lj_energy', 'special_pair_lj_energy',
                'special_pair_coul_energy', 'pair_ewald_energy',
                'pppm_energy'],
    period=100, overwrite=True)

hoomd.dump.gsd("dump.gsd", period=100, group=hoomd.group.all(), overwrite=True)
