...
namespace SSAGES
{
	...
	private:
		...
		//!< Coordinates for comparison with reference structure
	...
	public:
		...
		void Initialize(const Snapshot& snapshot) override
		{
			...
			pdbfile.open(_refpdb, std::ios::in);
			std::string line;

			while(std::getline(pdbfile, line)){
				std::string record = line.substr(0, 6);
				unsigned int atomnum = std::stoul(line.substr(6, 5));
				std::string atomtype = line.substr(12, 4);
				std::string resname = line.substr(17, 3);
				std::string chain = line.substr(21, 1);
				unsigned int resnum = std::stoul(line.substr(22, 4));
				if((record == "ATOM") && ((resnum - _resids[0]) <= (_resids[-1] - _resids[0]))){
					if(resname == "GLY" && std::find(glycineAtoms.begin(), glycineAtoms.end(), atomtype) != glycineAtoms.end()){
						...
						resnames.push_back(resname);
						chains.push_back(chain);
						resnums.push_back(resnum); // ????? is there a better way
					}
				}
			}
			...
			for(size_t i = 0; i < _resids.size(); i++){
				...
				_atomids.insert(_atomids.end(), tempatoms.begin(), tempatoms.end());
			}

			// check that correct # atom ids read in (5 per residue)
			if(_atomids.size() != 5 * _resids.size()){
				std::cout << "" << std::endl;
				exit(0);
			}
			...
			// are snapshot values stored as nm or angstrom?
			//_refalpha[0] = Vector3{ 0.733,  0.519,  5.298 }; // N    
			//_refalpha[1] = Vector3{ 1.763,  0.810,  4.301 }; // CA
			//_refalpha[2] = Vector3{ 3.166,  0.543,  4.881 }; // CB
			//_refalpha[3] = Vector3{ 1.527, -0.045,  3.053 }; // C
			//_refalpha[4] = Vector3{ 1.646,  0.436,  1.928 }; // O
			//_refalpha[5] = Vector3{ 1.180, -1.312,  3.254 }; // N    
			//_refalpha[6] = Vector3{ 0.924, -2.203,  2.126 }; // CA
			//_refalpha[7] = Vector3{ 0.650, -3.626,  2.626 }; // CB
			//_refalpha[8] = Vector3{-0.239, -1.711,  1.261 }; // C
			//_refalpha[9] = Vector3{-0.190, -1.815,  0.032 }; // O
			//_refalpha[10] = Vector3{-1.280, -1.172,  1.891 }; // N   
			//_refalpha[11] = Vector3{-2.416, -0.661,  1.127 }; // CA
			//_refalpha[12] = Vector3{-3.548, -0.217,  2.056 }; // CB
			//_refalpha[13] = Vector3{-1.964,  0.529,  0.276 }; // C
			//_refalpha[14] = Vector3{-2.364,  0.659, -0.880 }; // O
			//_refalpha[15] = Vector3{-1.130,  1.391,  0.856 }; // N   
			//_refalpha[16] = Vector3{-0.620,  2.565,  0.148 }; // CA
			//_refalpha[17] = Vector3{ 0.228,  3.439,  1.077 }; // CB
			//_refalpha[18] = Vector3{ 0.231,  2.129, -1.032 }; // C
			//_refalpha[19] = Vector3{ 0.179,  2.733, -2.099 }; // O
			//_refalpha[20] = Vector3{ 1.028,  1.084, -0.833 }; // N   
			//_refalpha[21] = Vector3{ 1.872,  0.593, -1.919 }; // CA
			//_refalpha[22] = Vector3{ 2.850, -0.462, -1.397 }; // CB
			//_refalpha[23] = Vector3{ 1.020,  0.020, -3.049 }; // C
			//_refalpha[24] = Vector3{ 1.317,  0.227, -4.224 }; // O
			//_refalpha[25] = Vector3{-0.051, -0.684, -2.696 }; // N   
			//_refalpha[26] = Vector3{-0.927, -1.261, -3.713 }; // CA
			//_refalpha[27] = Vector3{-1.933, -2.219, -3.074 }; // CB
			//_refalpha[28] = Vector3{-1.663, -0.171, -4.475 }; // C
			//_refalpha[29] = Vector3{-1.916, -0.296, -5.673 }; // O
