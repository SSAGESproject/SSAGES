 ...
 */
 ...
 namespace SSAGES
 {
    ...
    //! Collective variable of nearest neighbors between two groups of atoms.
	/*!
	 * Collective variable on nearest neighbors between two groups of atoms.
	 *
	 * \ingroup CVs
	 */
	...
	private:
		...
		GaussianFunction gf_; //!< Gaussian function used for counting neighbors.
	...
	public:
		...
		void Evaluate(const Snapshot& snapshot) override
		{
			...
			// More efficient implementation, but not yet working
			// a la CoordinationNumberCV.h
			// for(auto& i : idx1)
			// {
			// 	auto& p = positions[i];
			// 	for(size_t j = 0; j < group1_.size(); ++j)
			// 	{
			...
			// Old implementation, double counts
			for(auto& i : idx1)
			{
				auto& p = positions[i];
				for(auto& j : idx1)
				{
					if(i != j)
					{
						...
						// Parallelization, Hythem needs to validate algorithm before implementation...
						// auto lidx = snapshot.GetLocalIndex(gids[j]);
						// if(lidx != -1)
						// 	grad_[lidx] -= df*rij/r;
					}
