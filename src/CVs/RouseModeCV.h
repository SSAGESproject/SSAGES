...
namespace SSAGES
{
	...
	public:
		...
		void Evaluate(const Snapshot& snapshot) override
		{
			...
			// Note: this solution is valid for homogeneous mass/friction-- can try to 
			// derive solution for heterogeneous mass, otherwise this is approximate
			// and controlled by the user definition of groups. Possible that
			// solution can be obtained through diagonalization of the Rouse matrix
			xp_.fill(0.0);
			for (size_t i = 0; i<N_; ++i) {
			...
			// dCV/dxjd = (Xpd/CV)*(c/N)**0.5*sum_i^N cos[p*pi(i-0.5)/N] mj/Mi *delta_j({i})
