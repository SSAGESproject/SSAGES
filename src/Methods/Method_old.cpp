...
namespace SSAGES
{
						...
						const std::string& path)
	{
		...
		else if(type == "ABF")
		{
			...
			N_[i] = new Grid<int>(N_->GetNumPoints(),N_->GetLower(),N_->GetUpper(),N_->GetPeriodic());
			Nworld_[i] = new Grid<int>(N_->GetNumPoints(),N_->GetLower(),N_->GetUpper(),N_->GetPeriodic());

			auto* m = new ABF(N_, Nworld_, world, comm, restraint, isperiodic, periodicboundaries, min, massweigh, filename, histdetails, FBackupInterv, unitconv, timestep, freq);
